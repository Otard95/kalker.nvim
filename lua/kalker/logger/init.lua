local levels = require("kalker.logger.level")
...
--- @param config KalkerOptions
function Logger:init(config)
  if config.log_level ~= nil then
    self.level = config.log_level
  end
  if config.log_file ~= nil then
    self:debug("[kalker.nvim](Logger::init)", "log_file", config.log_file)
    self:file_sink(config.log_file)
  end
end
...
local module_logger = Logger:new(levels.ERROR)
